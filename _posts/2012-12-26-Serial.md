...
StaticJsonDocument<BUFFER_SIZE> json; // a single global buffer for json
String buffer = "";
...
void handleCommand(const String &buffer)
{
  ...
  if (error)
  {
    Serial.println("A parsing error occurred");
    return;
  }

  // parse a command such as: {"value": 123}
  if (json.containsKey(F("value")))
  {
    uint16_t analogVal = json["value"];
    uint8_t mappedValue= map (analogVal, 0, 1024, 0, 256);
